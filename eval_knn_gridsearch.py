...
GRID_K       = [3, 5, 7]
GRID_P       = [1.0, 2.0, 3.0]
GRID_TAU     = [None, 0.60, 0.65, 0.70]
GRID_MARGIN  = [0.00, 0.05, 0.10]
...
def evaluate_titles(titles, y_true, K=5, P=2.0, TAU=None, MARGIN=0.0):
    """
    評估指定參數下的表現；回傳字典含各種指標與時間。
    """
    y_pred = []
    conf = defaultdict(Counter)
    enc_times, qry_times = [], []

    for title, tlabel in zip(titles, y_true):
        pred, sims, knn_labels, t_enc, t_qry = predict_one(title, K, P, TAU, MARGIN)
        y_pred.append(pred)
        conf[tlabel][pred] += 1
...
def grid_search_knn(titles, y_true):
    """
    在測試集上做 grid search（會對該集「樂觀」；正式流程建議在訓練集做 CV）。
    回傳最佳參數與對應分數。
    """
    best, best_params = None, None
    print("=== Grid Search 開始 ===")
    for K, P, TAU, MARGIN in product(GRID_K, GRID_P, GRID_TAU, GRID_MARGIN):
        res = evaluate_titles(titles, y_true, K, P, TAU, MARGIN)
        print(f"K={K} P={P} TAU={TAU} MARGIN={MARGIN} | "
              f"macroF1={res['macro_f1']:.4f} acc={res['accuracy']:.4f} cov={res['coverage']:.4f}")
        # 以 macro-F1 為主，若持平則以 coverage 較高者優先
        key = (res["macro_f1"], res["coverage"])
        if (best is None) or (key > best[0]):
            best = (key, (K, P, TAU, MARGIN))
    (best_macro, best_cov), (bk, bp, btau, bmar) = best
    print(f"=== Grid Search 完成：最佳參數 K={bk} P={bp} TAU={btau} MARGIN={bmar} | "
          f"macroF1={best_macro:.4f} cov={best_cov:.4f} ===")
    return {"K": bk, "P": bp, "TAU": btau, "MARGIN": bmar}

def main():
    # 載入測試資料
    titles, y_true = load_test(TEST_JSONL)

    # Grid Search
    t0 = time.perf_counter()
    best_params = grid_search_knn(titles, y_true)
    t1 = time.perf_counter()

    # 用最佳參數做一次完整評估（含時間統計與表格）
    res = evaluate_titles(titles, y_true, **best_params)
    t2 = time.perf_counter()
    ...
    print("\n=== 執行時間（秒）===")
    print(f"Grid Search 總耗時: {t1 - t0:.3f}")
    print(f"最終評估 總耗時: {t2 - t1:.3f}")
